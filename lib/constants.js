...
const PHP_REQUIREMENT_RE = /^>=\d+\.\d+(?:\.\d+)?$/;
...
// Derives the beta-channel URL from a base by adding channel=beta as a query param, so the
// existing VERSION_CHECK_URL/API_URL test seams keep working unchanged for both channels.
const betaChannelUrl = (base) => {
  const u = new URL(base);
  u.searchParams.set('channel', 'beta');
  return u.href;
};
